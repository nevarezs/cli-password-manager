...
Authentication is used to provide integrity in addition to confidentiality. This allows the program to detect if an attacker tampered with the ciphertext. According to [Katz and Lindell][5]: *“It is best practice to always encrypt and authenticate by default; encryption alone should not be used unless there are compelling reasons to do so (such as implementations on severely resource-constrained devices) and, even then, only if one is absolutely sure that no damage can be caused by undetected modification of data.”*

**Why Am I Using So Many Keys?**
The reason is to follow [one of the cryptographic principles][5]: *“different security goals should always use different keys.”* This means that the keys used for encryption should be different from the keys used for authentication. Also following the same principle, the key to encrypt the database should be different from the key used to encrypt passwords. This script even uses a different key to encrypt each password. This is because the key used to encrypt a password is derived from the master password and a random salt. The random salt is different each time that the set of keys are generated, so a different key is created.
...
[1]: http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf "Announcing the ADVANCED ENCRYPTION STANDARD (AES)"

[5]: https://www.crcpress.com/Introduction-to-Modern-Cryptography-Second-Edition/Katz-Lindell/p/book/9781466570269 "J. Katz and Y.Lindell, Introduction to Modern Cryptography: Principles and Protocols, Chapman & Hall/CRC Press, August 2007."

[7]: http://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-132.pdf "NIST Special Publication 800-132. Recommendation for Password-Based Key Derivation Part 1: Storage Applications"

[8]: https://www.owasp.org/index.php/Password_Storage_Cheat_Sheet "Password Storage Cheat Sheet"

[13]: http://csrc.nist.gov/publications/fips/fips198-1/FIPS-198-1_final.pdf "The Keyed-Hash Message Authentication Code (HMAC)"

[16]: http://www.di-mgt.com.au/cryptopad.html "Using Padding in Encryption"

[17]: http://www.tldp.org/HOWTO/Shadow-Password-HOWTO-2.html "Why shadow your passwd file?"
